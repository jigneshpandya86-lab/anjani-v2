...
async function testCreateManualPost() {
  ...
  try {
    const docRef = await db.collection('googleBusinessPosts').add({
      summary:
        '🌟 Fresh pure water delivered to your doorstep! Order now and enjoy 15% discount on bulk orders. Call us: 9925997750 #PureWater #VadodaraWater',
...
async function testRetrievePendingPosts() {
  ...
  try {
    const snapshot = await db
      .collection('googleBusinessPosts')
